module ApplicationHelper
  def cheak_use_picture(user, size)
    if user.profile_picture.attached?
      link_to (image_pack_tag user.profile_picture.variant(gravity: :center, resize:"640x640^", crop:"640x640+0+0").processed, size: size, alt: "icon", class: "img-fluid rounded-circle"), user_path(user)
    else
